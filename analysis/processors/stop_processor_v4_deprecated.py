...
class AnalysisProcessor(processor.ProcessorABC):
    ...
    def process(self, events):
        ...
        e['hlt_sf'] = get_ele_hlt_sf(self._year, e.eta, e.pt, e.phi)
        e['T'] = ak.zip({
            'r': e.pt,
            'phi': e.phi,
        ...
        fj_clean_p = fj_good[fj_good.isclean_l]
        ...
        scalarHT = {
            ...
            'cat4_GCR_highDeltaM': ak.sum(j_good.pt, axis=1),
            'cat5_DY2E_highDeltaM': ak.sum(j_good.pt, axis=1),
            'cat6_DY2M_highDeltaM': ak.sum(j_good.pt, axis=1),
        ...
        regions = {
            ...
            'cat4_GCR_highDeltaM': [
                ...
                'zero_m', 'zero_t', 'zero_e', 'five_j_clean_l', 'one_b',
            ...
            'cat5_DY2E_highDeltaM': [
                ...
                'electron_trigger',
                'zero_trk_e', 'zero_trk_m', 'zero_trk_pi',
                'zero_t','five_j', 'one_b', 'zero_m', 'two_e',
            ...
            'cat6_DY2M_highDeltaM': [
                ...
                'muon_trigger',
                'zero_trk_e', 'zero_trk_m', 'zero_trk_pi',
                'zero_t','five_j', 'one_b', 'zero_e', 'two_m',
        ...
        if not isData:
            ...
            if ('TTto' in dataset):
                top_pt_sf = ak.to_numpy(
                    np.sqrt(get_top_pt_reweight(genTops.pt[:, 0]) * get_top_pt_reweight(genTops.pt[:, 1]))
                )
        ...
        def fill(region, systematic, region_weights):
            ...
            if region_weights is None:
                ...
                weight = region_weights.weight()[cut]

            # histogram key -> (axis name, value)
            fill_map = {
                'template': ('met', met.pt),
                'metpt': ('met', met.pt),
                'metpt_10GeVbins': ('metpt_10GeVbins', met.pt),
                'metphi': ('metphi', met.phi),
                'recoilpt': ('recoilpt', uT[region].r),
                'recoilphi': ('recoilphi', uT[region].phi),
                'nElectron': ('nElectron', n_e_medium),
                'nMuon': ('nMuon', n_m_medium),
                'nJet': ('nJet', n_j_good),
                'j1pt': ('j1pt', j1.pt),
                'j1eta': ('j1eta', j1.eta),
                'j1phi': ('j1phi', j1.phi),
                ...
                'j2eta': ('j2eta', j2.eta),
                'j2phi': ('j2phi', j2.phi),
                'nb_loose': ('nb_loose', nb_loose),
                'nb_medium': ('nb_medium', nb_medium),
                'nb_tight': ('nb_tight', nb_tight),
                'b1_medium_pt': ('b1_medium_pt', b1.pt),
                'b1_medium_eta': ('b1_medium_eta', b1.eta),
                'b1_medium_phi': ('b1_medium_phi', b1.phi),
                'b1_tight_pt': ('b1_tight_pt', b1_tight.pt),
                'b1_tight_eta': ('b1_tight_eta', b1_tight.eta),
                'b1_tight_phi': ('b1_tight_phi', b1_tight.phi),
                'b1_loose_pt': ('b1_loose_pt', b1_loose.pt),
                'b1_loose_eta': ('b1_loose_eta', b1_loose.eta),
                'b1_loose_phi': ('b1_loose_phi', b1_loose.phi),
                'ht': ('ht', scalarHT[region]),
                'nPV': ('nPV', npv),
                'nfj': ('nfj', nfj_good),
                'fj1pt': ('fj1pt', ak.fill_none(ak.firsts(fj_good).pt, -99)),
                'fj1mass': ('fj1mass', ak.fill_none(ak.firsts(fj_good).mass, -99)),
                'fj1msd': ('fj1msd', ak.fill_none(ak.firsts(fj_good).msoftdrop, -99)),
                'fj1phi': ('fj1phi', ak.fill_none(ak.firsts(fj_good).phi, -99)),
                'fj1eta': ('fj1eta', ak.fill_none(ak.firsts(fj_good).eta, -99)),
                'fj1TvsQCD': ('fj1TvsQCD', ak.fill_none(ak.firsts(fj_good).particleNetWithMass_TvsQCD, -99)),
                'fj1WvsQCD': ('fj1WvsQCD', ak.fill_none(ak.firsts(fj_good).particleNetWithMass_WvsQCD, -99)),
                'fj1QCD': ('fj1QCD', ak.fill_none(ak.firsts(fj_good).particleNetWithMass_QCD, -99)),
            }

            if region in mll:
                fill_map['mll'] = ('mll', mll[region])
            if region in pll:
                fill_map['pll'] = ('pll', pll[region])

            for hist_key in output:
                if hist_key == 'sumw':
                    continue
                if hist_key not in fill_map:
                    continue

                axis_name, values = fill_map[hist_key]

                fill_kwargs = {
                    'region': region,
                    'systematic': sys_name,
                    axis_name: normalize(values, cut),
                    'weight': weight,
                }

                # metpt histogram만 signal_trigger 축이 추가로 있음
                if hist_key == 'metpt':
                    fill_kwargs['signal_trigger'] = signal_triggers[cut]

                output[hist_key].fill(**fill_kwargs)
        ...
        for region in regions:
            ...
            for systematic in systematics:
                if isData and systematic is not None:
                    continue
                fill(region, systematic, region_weights)
