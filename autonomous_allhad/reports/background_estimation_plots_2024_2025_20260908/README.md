...
2024와 2025 모두 2026-09-11 승격된 새 Top/W SF 히스토그램으로 재측정한 결과입니다. 두 연도 최종 감사가 완료되었으며, 이미 완료된 2025 계산·플롯은 반복하지 않았습니다.
...
- 기존 plot_recoil_transfer_factors_2024.py를 재사용합니다. 두 연도 모두 새 히스토그램 유래 TF 입력으로 재측정·그린 결과이며, 저장된 TF JSON의 --plot-only 재렌더링은 측정값을 변경하지 않습니다.
- 카테고리별 낱장을 통합 그림으로 교체했습니다. 두 연도 원본 측정과 그림이 각각 새 입력 해시의 결과로 갱신되었습니다.
