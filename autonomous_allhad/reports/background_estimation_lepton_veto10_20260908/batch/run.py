...
def main():
    ...
    if year not in ('2024', '2025'):
        raise ValueError('expected campaign year')
    ...
    if state_path.exists():
        ...
        if previous.get('status') == 'complete' and previous.get('inputs') == inputs and previous.get('code_manifest_sha256') == sha(CAMPAIGN / 'batch/code_manifest.json') and all(sha(output / p) == h for p, h in previous['products'].items()):
            ...
            return 0
        raise ValueError('existing calculation state is not reusable; diagnose instead of overwriting')
    ...
    try:
        for command in commands:
            subprocess.run([sys.executable] + command, cwd=str(REPO), env=env, check=True)
            state['steps'].append({'command': command, 'exit_code': 0})
            state_path.write_text(json.dumps(state, indent=2) + '\n')
        names = ('dy_measurement.json', 'sgamma/sgamma_ut.json', 'sgamma/sgamma_ut.csv', 'zgamma/zgamma_double_ratio.json', 'tf_inputs.json')
        state.update(status='complete', products={p: sha(output / p) for p in names}, finished_unix=time.time())
