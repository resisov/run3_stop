...
def plot_factors(factors: dict[str, Any], output_dir: Path) -> list[str]:
    """Reuse the existing figures for measured factors; no refit is performed."""
    paths = zinv.plot_q(factors, output_dir)
    for regime in ("highdm", "lowdm"):
        paths.extend(zinv.plot_sgamma(factors[regime], regime, output_dir, TAIL_MERGED_EDGES[regime]))
    return paths
...
def main() -> int:
    ...
    rendering.add_argument("--no-plots", action="store_true", help="Calculate on a batch worker without rendering figures.")
    rendering.add_argument("--plot-only", action="store_true", help="Render the existing factor JSON without changing or refitting it.")
    parser.add_argument(
    ...
    if args.plot_only:
        product = args.output_dir / "sgamma_ut.json"
        payload = json.loads(product.read_text())
        if payload.get("status") != "complete" or payload["provenance"]["hist_input_sha256"] != file_sha256(args.hist_input) or payload["provenance"]["campaign_year"] != args.campaign_year:
            raise ValueError("Sgamma plot-only input is stale or belongs to a different year")
        factors = {"highdm": payload["highdm"], "lowdm": payload["lowdm_families"], "lowdm_Q_groups": payload["lowdm_Q_groups"]}
        paths = plot_factors(factors, args.output_dir)
        zinv.write_json(args.output_dir / "plot_manifest.json", {"source_sha256": file_sha256(product), "plots": paths})
        print(json.dumps({"status": "complete", "plot_only": True, "plots": paths}))
        return 0
    ...
    plot_paths = [] if args.no_plots else plot_factors(factors, args.output_dir)
    payload = {
