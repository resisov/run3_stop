...
def main() -> int:
    ...
    rendering.add_argument("--no-plots", action="store_true", help="Calculate without rendering figures on the batch worker.")
    rendering.add_argument("--plot-only", action="store_true", help="Render the measured JSON without changing or recalculating factors.")
    parser.add_argument(
    ...
    if args.plot_only:
        product = args.output_dir / "zgamma_double_ratio.json"
        payload = json.loads(product.read_text())
        if payload.get("status") != "complete" or payload["provenance"]["hist_input_sha256"] != sha256(args.hist_input) or payload["provenance"]["campaign_year"] != args.campaign_year or payload["provenance"]["low_ut_min_gev"] != args.low_ut_min:
            raise ValueError("Double-ratio plot-only input is stale or belongs to a different year/domain")
        paths = []
        for regime in ("highdm", "lowdm"):
            paths.extend(plot(regime, payload[regime]["bins"], args.output_dir))
        (args.output_dir / "plot_manifest.json").write_text(json.dumps({"source_sha256": sha256(product), "plots": paths}, indent=2) + "\n")
