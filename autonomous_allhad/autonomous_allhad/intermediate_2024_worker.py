...
def _passthrough_jec(
    ...
    return pt, mass, {
        ...
        "applied": True,
        "source": "object_corrections_2024_precalibrated",
...
def _passthrough_met(
    ...
    return (
        ...
        np.asarray(arrays["PuppiMET_phi"], dtype=float),
        {
            ...
            "shift": validate_shift(shift_name),
            "applied": True,
            "source": "object_corrections_2024_precalibrated_and_propagated",
        },
    )


def extract_chunk_2024(
    arrays: Any,
    dataset: str,
    process: str,
    sp: str | None,
    ...
    decorate_rows: bool = True,
    materialize_skim_flag: str | None = None,
    precalibrated: tuple[Any, dict[str, Any]] | None = None,
    ...
    audit = (
        ...
        else branch_audit(set(arrays.fields), is_data=is_data)
    )
    if not isinstance(audit, dict):
        raise RuntimeError("prevalidated object branch audit is missing")
    if audit["status"] != "valid":
        raise RuntimeError(f"required 2024 object branches missing: {audit['missing_required']}")
    root = Path.cwd()
    payload_status = (
        validation_context.get("payload_status")
        if validation_context is not None
        else validate_payloads(root)
    )
    if not isinstance(payload_status, dict):
        raise RuntimeError("prevalidated correction payload status is missing")
    if payload_status["status"] != "valid":
        raise RuntimeError(f"2024 correction payload validation failed: {payload_status['errors']}")
    if precalibrated is None:
        ...
        corrected, calibration = precalibrated
        if str(calibration.get("shift")) != validate_shift(shift_name):
            raise RuntimeError(
                f"precalibrated shift mismatch: {calibration.get('shift')} vs {validate_shift(shift_name)}"
            )
    ...
    summary["photon_selection"] = "medium photon with explicit Photon_electronVeto; photon-cleaned AK4/AK8 jets and photon-recoil direction"
    return rows, summary
...
def install_backend() -> None:
    ...
    flat.extract_chunk = extract_chunk_2024
