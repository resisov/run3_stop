...
OLD_SELECTED_SCHEME = "boosted_an17_selected_recoil6_with_nt0_SR"
NEW_SELECTED_SCHEME = "boosted_an17_selected_recoil6_with_nt0_wsplit_SR"
INSERT_AFTER_BINS = len(RECOIL_BIN_LABELS)
NEW_CATEGORY_KEY = "Nb1plus_T0_W1plus"
NEW_CATEGORY_LABEL = "$N_{b}\\geq1$, $N_{t}=0$\n$N_{W}\\geq1$"
NEW_CATEGORY_PREFIX = "NT0_Nb1plus_T0_W1plus"
...
def selected_nt0_recoil_indices(chunk: dict[str, Any], n: int) -> np.ndarray:
    ...
    mask = sr & (nb >= 1) & (nt == 0) & (nw >= 1) & (recoil_idx >= 0) & (recoil_idx < len(RECOIL_PT_BINS) - 1)
...
def insert_record(old_rec: dict[str, Any], new_rec: dict[str, Any]) -> dict[str, Any]:
    return {
        "sumw": list(old_rec.get("sumw") or [])[:INSERT_AFTER_BINS] + list(new_rec.get("sumw") or []) + list(old_rec.get("sumw") or [])[INSERT_AFTER_BINS:],
        "sumw2": list(old_rec.get("sumw2") or [])[:INSERT_AFTER_BINS] + list(new_rec.get("sumw2") or []) + list(old_rec.get("sumw2") or [])[INSERT_AFTER_BINS:],
        "entries": list(old_rec.get("entries") or [])[:INSERT_AFTER_BINS] + list(new_rec.get("entries") or []) + list(old_rec.get("entries") or [])[INSERT_AFTER_BINS:],
    }
...
def build_combined_scheme(base: dict[str, Any], new6: dict[str, Any]) -> dict[str, Any]:
    ...
    for sample in sorted(set(old_by_sample) | set(new6)):
        ...
        for vname in vnames:
            old_rec = padded_record(old_vars, vname, 48)
            new_rec = padded_record(new_vars, vname, 6)
            combined.setdefault(sample, {})[vname] = insert_record(old_rec, new_rec)
    return combined


def main() -> int:
    parser = argparse.ArgumentParser(description="Insert the requested Nt=0,NW>=1 recoil category after the existing Nt=0,NW=0 selected recoil block.")
    ...
    if len(old_labels) != 48:
        raise ValueError(f"expected 48 old selected labels, got {len(old_labels)}")
    ...
    base.setdefault("search_bin_schemes", {})[NEW_SELECTED_SCHEME] = {
        "bin_labels": old_labels[:INSERT_AFTER_BINS] + new_labels + old_labels[INSERT_AFTER_BINS:],
        ...
        "inserted_category": {"key": NEW_CATEGORY_KEY, "label": NEW_CATEGORY_LABEL, "insert_after_bins": INSERT_AFTER_BINS, "recoil_pt_bins": RECOIL_PT_BINS},
        "source_scheme_for_existing_48_bins": OLD_SELECTED_SCHEME,
    ...
    base.setdefault("summary", {}).setdefault("derived_search_bin_schemes", {})[NEW_SELECTED_SCHEME] = {
        ...
        "insert_after_bins": INSERT_AFTER_BINS,
        "new_category_selected_entries": int(sum(s.get("selected_entries", 0) for s in summaries)),
        "workers": summaries,
        "elapsed_seconds": time.time() - start,
    }
    write_json(Path(args.output), base)
    print(json.dumps({"status": "complete", "scheme": NEW_SELECTED_SCHEME, "bins": 54, "new_category_entries": int(sum(s.get("selected_entries", 0) for s in summaries)), "output": args.output, "elapsed_seconds": time.time() - start}, sort_keys=True))
    return 0
