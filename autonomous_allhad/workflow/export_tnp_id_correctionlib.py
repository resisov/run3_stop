...
from workflow.sf_payload import (
    correction,
    correction_set,
    ensure_adopted,
    write_json_gz,
...
def main() -> int:
    ...
    parser.add_argument(
        "--candidate",
        action="store_true",
        help="write a validation-pending candidate beside measurement outputs; never install it under analysis/data",
    )
    parser.add_argument(
    ...
    if args.candidate:
        try:
            args.output.resolve().relative_to((Path.cwd() / "analysis/data").resolve())
        except ValueError:
            pass
        else:
            raise ValueError("candidate payloads must not be written below analysis/data")
    else:
