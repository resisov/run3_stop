...
ROBUST_FIT=${ROBUST_FIT:-1}
SAVE_FIT_SHAPES=${SAVE_FIT_SHAPES:-0}
...
case "$ROBUST_FIT" in
    0|1) ;;
    *) echo "ROBUST_FIT must be 0 or 1" >&2; exit 2 ;;
esac

case "$SAVE_FIT_SHAPES" in
    0|1) ;;
    *) echo "SAVE_FIT_SHAPES must be 0 or 1" >&2; exit 2 ;;
esac
...
if [[ "$SAVE_FIT_SHAPES" == 1 ]]; then
    COMBINE_ARGS+=(--saveShapes --saveWithUncertainties)
fi
...
python3 "$(dirname "$0")/extract_cronly_fit_covariance.py" \
    --fit-diagnostics "$OUTPUT_DIR/fitDiagnostics_cronly_2024_2025.root" \
    --output "$OUTPUT_DIR/fit_parameters.json"
