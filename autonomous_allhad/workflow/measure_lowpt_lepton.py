...
from workflow.build_tnp_das_records import build_records  # noqa: E402
from workflow.audit_lowpt_reference_triggers import audit  # noqa: E402
from workflow.export_tnp_id_correctionlib import build_payload  # noqa: E402
from workflow.prepare_tnp_measurement_condor import prepare  # noqa: E402
from workflow.reference_trigger_counts import json_safe  # noqa: E402
from workflow.sf_payload import ensure_adopted, write_json_gz  # noqa: E402
from workflow.tnp_fit import fit_histogram_payload  # noqa: E402
from workflow.tnp_histograms import build_histograms, read_file_list  # noqa: E402
from workflow.tnp_measurement_reduce import (  # noqa: E402
    merge_tnp_shards,
    rebin_probe_histograms,
    use_mc_reference,
)
from workflow.tnp_measurement_shard import count_tnp_shard  # noqa: E402
from workflow.tnp_recovery import (  # noqa: E402
    build_residual_manifest,
    finalize_permanent_skips,
    recover,
)
from workflow.validate_tnp_adoption import validate  # noqa: E402
...
DEFAULT_CONFIGS = {
    ("electron", "2024"): Path("autonomous_allhad/workflow/lowpt_electron_measurement/config_2024_id_only_parking_singlemuon.json"),
    ("electron", "2025"): Path("autonomous_allhad/workflow/lowpt_electron_measurement/config_2025_id_only_parking_singlemuon.json"),
    ("muon", "2024"): Path("autonomous_allhad/workflow/lowpt_muon_measurement/config_2024_id_only_parking_external.json"),
    ("muon", "2025"): Path("autonomous_allhad/workflow/lowpt_muon_measurement/config_2025_id_only_parking_external.json"),
}
REQUIRED_ASSETS = (
    Path("analysis/data/lumiMask/Cert_Collisions2024_378981_386951_Golden.json"),
    Path("analysis/data/lumiMask/Cert_Collisions2025_391658_398903_Golden.json"),
...
RELEASE_FILES = (
    Path("autonomous_allhad/workflow/measure_lowpt_lepton.py"),
    Path("autonomous_allhad/workflow/audit_lowpt_reference_triggers.py"),
    Path("autonomous_allhad/workflow/build_tnp_das_records.py"),
    Path("autonomous_allhad/workflow/prepare_tnp_measurement_condor.py"),
    Path("autonomous_allhad/workflow/tnp_histograms.py"),
    Path("autonomous_allhad/workflow/tnp_measurement_shard.py"),
    Path("autonomous_allhad/workflow/reference_trigger_counts.py"),
    Path("autonomous_allhad/workflow/tnp_recovery.py"),
    Path("autonomous_allhad/workflow/tnp_measurement_reduce.py"),
    Path("autonomous_allhad/workflow/tnp_fit.py"),
    Path("autonomous_allhad/workflow/validate_tnp_adoption.py"),
    Path("autonomous_allhad/workflow/export_tnp_id_correctionlib.py"),
    Path("autonomous_allhad/workflow/plot_measurement.py"),
    Path("autonomous_allhad/workflow/sf_payload.py"),
    Path("autonomous_allhad/lowpt_tnp/README.md"),
    Path("autonomous_allhad/lowpt_tnp/environment.yml"),
    Path("autonomous_allhad/lowpt_tnp/requirements-py38.txt"),
    Path("autonomous_allhad/lowpt_tnp/requirements-modern.txt"),
    Path("autonomous_allhad/tests/test_lowpt_tnp_release.py"),
    Path(".github/workflows/lowpt-tnp.yml"),
    Path("autonomous_allhad/workflow/lowpt_electron_measurement/README.md"),
    Path("autonomous_allhad/workflow/lowpt_muon_measurement/README.md"),
    *DEFAULT_CONFIGS.values(),
    Path("autonomous_allhad/lowpt_tnp/records/electron_2025_data.json.gz"),
    Path("autonomous_allhad/lowpt_tnp/records/muon_2025_data.json.gz"),
    Path("autonomous_allhad/lowpt_tnp/records/electron_2024_mc.json.gz"),
    Path("autonomous_allhad/lowpt_tnp/records/muon_2024_mc.json.gz"),
    Path("autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/results/electron/histograms.json"),
    Path("autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/results/electron/fit_result.json"),
    Path("autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/results/muon/histograms.json"),
    Path("autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/results/muon/fit_result.json"),
    Path("autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/payloads/veto_electron_5to10_sf.json.gz"),
    Path("autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/payloads/loose_muon_5to10_sf.json.gz"),
    *REQUIRED_ASSETS,
)
RELEASE_GLOBS = (
    "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/electron/*.png",
    "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/electron/*.pdf",
    "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/electron/plot_manifest.json",
    "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/muon/*.png",
    "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/muon/*.pdf",
    "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/muon/plot_manifest.json",
)
...
def _release_files(repo: Path) -> tuple[Path, ...]:
    expanded = list(RELEASE_FILES)
    for pattern in RELEASE_GLOBS:
        expanded.extend(path.relative_to(repo) for path in repo.glob(pattern))
...
def _plot_tnp_result(
    ...
    *,
    ...
    from workflow.plot_measurement import plot_tnp_result
...
def build_release_manifest(repo: Path) -> dict[str, Any]:
    ...
    for path in release_files:
        if path.parent != Path("autonomous_allhad/lowpt_tnp/records"):
            continue
    ...
    for kind in KINDS:
        base = repo / "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/results" / kind
    ...
    for filename in ("veto_electron_5to10_sf.json.gz", "loose_muon_5to10_sf.json.gz"):
        path = repo / "autonomous_allhad/reports/lowpt_id_sf_2025_an_handoff/payloads" / filename
    ...
    return {
        ...
        "entrypoint": "autonomous_allhad/workflow/measure_lowpt_lepton.py",
...
def _parser() -> argparse.ArgumentParser:
    ...
    command.add_argument("--repo", type=Path, default=REPOSITORY_ROOT)
...
def main(argv: list[str] | None = None) -> int:
    ...
    if args.command == "select-records":
        ...
        result = build_release_manifest(args.repo)
        _write_json(args.output, result)
    elif args.command == "verify-release":
        result = verify_release(args.repo, args.manifest)
        print(json.dumps(result, indent=2, sort_keys=True))
