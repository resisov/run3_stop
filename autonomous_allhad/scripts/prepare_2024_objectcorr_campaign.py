...
DEFAULT_CAMPAIGN = DEFAULT_REPO / "autonomous_allhad/workflow/intermediate_2024_objectcorr_v1_20260721"
...
def make_shards(campaign: Path, records: list[dict[str, Any]], data_size: int, mc_size: int) -> list[dict[str, Any]]:
    ...
    for group_name, selected, size in groups:
        for index, start in enumerate(range(0, len(selected), size)):
            ...
            atomic_json(path, {
                "schema_version": "full_production_shard_spec_v3_objectcorr_2024",
                "shard_id": name,
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="Prepare the corrected 2024 intermediate ROOT campaign.")
    ...
    campaign_manifest = {
        "schema_version": "intermediate_2024_objectcorr_campaign_v2",
    ...
    report = (
        "# 2024 Intermediate ROOT Campaign\n\n"
