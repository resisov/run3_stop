...
DEFAULT_NOMINAL_CAMPAIGN = DEFAULT_REPO / "autonomous_allhad/workflow/intermediate_2024_objectcorr_v1_20260721"
DEFAULT_CAMPAIGN = DEFAULT_REPO / "autonomous_allhad/workflow/shape_hists_2024_finaljes11_v1_20260721"
...
set +e
"@D@PY" -u -m autonomous_allhad.shape_histogram_2024_worker   --shard "@D@WORKDIR/@D@SHARD"   --output "@D@WORKDIR/out.json.gz"   --metadata-output "@D@WORKDIR/out.meta.json"   --variation-group all --chunk-size 50000
WORKER_STATUS="@D@?"
set -e
...
queue name,shard,shard_name,hist_out,meta_out from {arguments}
"""


def build_mc_shards(
    nominal_campaign: Path,
    campaign: Path,
    files_per_job: int,
) -> tuple[list[Path], dict[str, int]]:
    source = nominal_campaign / "shards"
    destination = campaign / "shards"
    destination.mkdir(parents=True, exist_ok=True)
    shards: list[Path] = []
    seen_files: set[str] = set()
    source_records = 0
    parent_shards = 0
    for parent in sorted(source.glob("mc_shard_*.json")):
        parent_payload = read_json(parent)
        records = list(parent_payload.get("records") or [])
        if not records:
            raise RuntimeError(f"empty parent MC shard: {parent}")
        parent_shards += 1
        for record in records:
            file_path = str(record.get("file_path") or "")
            if not file_path:
                raise RuntimeError(f"record without file_path in {parent}")
            if file_path in seen_files:
                raise RuntimeError(f"duplicate MC file across parent shards: {file_path}")
            seen_files.add(file_path)
        source_records += len(records)
        for start in range(0, len(records), files_per_job):
            child_records = records[start : start + files_per_job]
            ...
            child_payload = {
                "schema_version": "shape_histogram_2024_input_shard_v2",
                "shard_id": child_id,
                "record_group": "mc_background",
                "record_digest": hashlib.sha256(canonical).hexdigest(),
                "source_parent_shard": str(parent_payload.get("shard_id") or parent.stem),
                "source_parent_record_digest": parent_payload.get("record_digest"),
                "source_record_range": [start, start + len(child_records)],
    ...
    if not shards:
        raise RuntimeError(f"no MC shards found under {source}")
    return shards, {
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("--request-memory-mb", type=int, default=12000)
    ...
    for directory in ("bundles", "condor", "logs", "outputs", "reports", "shards", "smoke_runtime"):
        (campaign / directory).mkdir(parents=True, exist_ok=True)

    shards, shard_stats = build_mc_shards(nominal_campaign, campaign, args.files_per_job)
    ...
    for index, shard in enumerate(shards):
        ...
        rows.append(f"{name} {shard} {shard.name} {hist_out} {meta_out}")

    arguments = campaign / "condor/arguments.txt"
    pilot_arguments = campaign / "condor/arguments_pilot.txt"
    remaining_arguments = campaign / "condor/arguments_remaining.txt"
    arguments.write_text("\n".join(rows) + "\n")
    pilot_arguments.write_text(rows[0] + "\n")
    remaining_arguments.write_text("\n".join(rows[1:]) + ("\n" if len(rows) > 1 else ""))

    pilot_submit = campaign / "condor/submit_pilot.sub"
    remaining_submit = campaign / "condor/submit_remaining.sub"
    ...
    pilot_submit.write_text(
        submit_text(arguments=pilot_arguments, **submit_common)
    )
    remaining_submit.write_text(
        submit_text(arguments=remaining_arguments, **submit_common)
    )
    ...
    manifest = {
        ...
        "source_policy": {
            ...
            "nano_chunk_read": "once per chunk followed by sequential variation evaluation",
        ...
        "submission": {
            "pool": "eossubmit",
            "pilot": f"module load lxbatch/eossubmit && condor_submit {pilot_submit}",
            "remaining_after_pilot_validation": f"module load lxbatch/eossubmit && condor_submit {remaining_submit}",
            "all": f"module load lxbatch/eossubmit && condor_submit {all_submit}",
            "pilot_submit_file": str(pilot_submit),
            "remaining_submit_file": str(remaining_submit),
...
- Search bins: final 54-bin high-dM SR plus six inclusive low-dM regions
- Detailed distributions: nominal CR/VR/SR region, variable, and bin definitions
- Legacy 17-bin and 42-bin schemes: excluded
- Runtime: transferred tgz bundles under Condor scratch
- Stageout: EOS through xrdcp
- Submission: full campaign only after smoke test, checksum validation, and AFS-free Condor dry-run
"""
